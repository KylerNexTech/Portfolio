    ...
    function setIcon(muted) {
      soundIcon.innerHTML = muted ? MUTED_ICON : UNMUTED_ICON;
    }

    function tryPlay() {
      if (userMuted) return;
      music.play().catch(() => {
        // Autoplay was blocked — it will start on first user interaction instead.
      });
    }
    ...
    // bc browsers block audio until the user interacts with the page at all.
    //  catches the very first interaction, then plays the music.
    ...
    // nav via click
    const DESTINATION = '../Home/home.html';
